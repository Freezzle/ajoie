...
type RestOf<T extends IPayment | NewPayment> = Omit<T, 'billingDate'> & {
...
export class PaymentService {
  ...
  protected convertDateFromClient<T extends IPayment | NewPayment | PartialUpdatePayment>(payment: T): RestOf<T> {
    return {
      ...payment,
      billingDate: payment.billingDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPayment: RestPayment): IPayment {
    return {
      ...restPayment,
      billingDate: restPayment.billingDate ? dayjs(restPayment.billingDate) : undefined,
    };
  }
