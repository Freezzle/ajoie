...
export class ConferenceService {
    ...
    partialUpdate(conference: PartialUpdateConference): Observable<EntityResponseType> {
        return this.http.patch<IConference>(`${this.resourceUrl}/${this.getConferenceIdentifier(conference)}`,
            conference, {
                observe: 'response',
            });
    }
    ...
    addConferenceToCollectionIfMissing<Type extends Pick<IConference, 'id'>>(
        ...
        if (conferences.length > 0) {
            const conferenceCollectionIdentifiers = conferenceCollection.map(
                conferenceItem => this.getConferenceIdentifier(conferenceItem));
