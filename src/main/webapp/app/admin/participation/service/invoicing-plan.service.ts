...
export class InvoicingPlanService {
  ...
  createInvoice(idInvoicingPlan: string, invoice: IInvoice): Observable<HttpResponse<IInvoice>> {
    return this.http.post<IInvoice>(`this.resourceUrl/${idInvoicingPlan}/invoices`, invoice, { observe: 'response' });
  }

  updateInvoice(idInvoicingPlan: string, invoice: IInvoice): Observable<HttpResponse<IInvoice>> {
    return this.http.put<IInvoice>(`this.resourceUrl/${idInvoicingPlan}/invoices/${invoice.id}`, invoice, { observe: 'response' });
  }

  createPayment(idInvoicingPlan: string, payment: IPayment): Observable<HttpResponse<IPayment>> {
    return this.http.post<IPayment>(`this.resourceUrl/${idInvoicingPlan}/payments`, payment, { observe: 'response' });
  }

  updatePayment(idInvoicingPlan: string, payment: IPayment): Observable<HttpResponse<IPayment>> {
    return this.http.put<IInvoice>(`this.resourceUrl/${idInvoicingPlan}/payments/${payment.id}`, payment, { observe: 'response' });
  }

  deletePayment(idInvoicingPlan: string, idPayment: string): Observable<HttpResponse<void>> {
    return this.http.delete<void>(`this.resourceUrl/${idInvoicingPlan}/payments/${idPayment}`, { observe: 'response' });
  }
  ...
  sendInvoiceReceipt(idInvoicingPlan: string): Observable<HttpResponse<{}>> {
    return this.http.patch<{}>(`this.resourceUrl/${idInvoicingPlan}/send-invoice-receipt`, {}, { observe: 'response' });
  }
}
