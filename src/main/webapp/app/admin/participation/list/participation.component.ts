...
export class ParticipationComponent implements OnInit {
    ...
    delete(participation: IParticipation): void {
        ...
        modalRef.componentInstance.translateValues = {description: participation.exhibitor?.fullName};
    ...
    protected onResponseSuccess(response: EntityArrayResponseType): void {
        ...
        if (statusFilter && statusFilter.length > 0) {
            this.participations = this.participations?.filter(
                participation => participation.status?.includes(statusFilter));
        }
    }
    ...
    protected queryBackend(): Observable<EntityArrayResponseType> {
        this.isLoading = true;
        return this.participationService.query(this.params.get('idSalon'))
