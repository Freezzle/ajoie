...
import {TpConfigRoomsComponent} from './tp-config-rooms/tp-config-rooms.component';
import {TpConfigDayComponent} from './tp-config-day/tp-config-day.component';
...
export class TpComponent {
    ...
    private sanitizeTalkAssignments(): void {
        ...
        this.talks.update(ts =>
                              ts.map(t => {
                                  if (t.dayId && !dayIds.has(t.dayId)) {
                                      return this.unassignTalk(t);
                                  }
                                  if (t.roomId && !roomIds.has(t.roomId)) {
                                      return this.unassignTalk(t);
                                  }
                                  if (t.dayId && t.roomId && !roomsByDayId.get(t.dayId)?.has(t.roomId)) {
                                      return this.unassignTalk(t);
                                  }
                                  return t;
                              })
    ...
    private computeDropSlot(
        ...
        if ((b.end - b.start) < periods) {
            return {slot: b.start, ok: false};
        }
        ...
        if (raw < min || raw > max) {
            return {slot: this.clamp(raw, min, max), ok: false};
        }
