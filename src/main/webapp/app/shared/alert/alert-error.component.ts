...
export class AlertErrorComponent implements OnDestroy {
    ...
    private addErrorAlert(message?: string, translationKey?: string,
                          translationParams?: { [key: string]: unknown }): void {
    ...
    private handleBadRequest(httpErrorResponse: HttpErrorResponse): void {
        ...
        if (errorHeader) {
            const alertData = entityKey ?
                {entityName: this.translateService.instant(`global.menu.entities.${entityKey}`)} : undefined;
    ...
    private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
        ...
        for (const fieldError of fieldErrors) {
            ...
            const fieldName: string = this.translateService.instant(
                `salonApp.${fieldError.objectName as string}.${convertedField}`);
