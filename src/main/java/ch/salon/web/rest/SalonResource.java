...
public class SalonResource {
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.ADMIN + "\")")
    public ResponseEntity<SalonDTO> updateSalon(@PathVariable(value = "id", required = false) final UUID id,
                                                @Valid @RequestBody SalonDTO salon) {
        log.debug("REST request to update Salon : {}, {}", id, salon);

        salon = salonService.update(id, salon);

        return ok().headers(createEntityUpdateAlert(applicationName, true, ENTITY_NAME, salon.getId().toString()))
            .body(salon);
    }
