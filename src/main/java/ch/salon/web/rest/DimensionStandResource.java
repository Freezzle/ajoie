...
public class DimensionStandResource {
    ...
    public ResponseEntity<DimensionStandDTO> createDimensionStand(
        @Valid @RequestBody DimensionStandDTO dimensionStand) throws URISyntaxException {
        log.debug("REST request to save DimensionStand : {}", dimensionStand);

        UUID id = dimensionStandService.create(dimensionStand);

        return created(new URI("/api/dimension-stands/" + id)).headers(
            createEntityCreationAlert(applicationName, true, ENTITY_NAME, id.toString())).body(dimensionStand);
    }
    ...
    public ResponseEntity<DimensionStandDTO> updateDimensionStand(
        ...
        return ok().headers(
                createEntityUpdateAlert(applicationName, true, ENTITY_NAME, dimensionStand.getId().toString()))
            .body(dimensionStand);
    }
