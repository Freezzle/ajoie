...
@Component
public class CancelParticipationHandler implements BusinessActionHandler<Participation> {
    ...
    public CancelParticipationHandler(EventLogService eventLogService, StandRepository standRepository,
            WorkshopRepository workshopRepository, ConferenceRepository conferenceRepository,
            InvoicingPlanRepository planRepository) {
    ...
    public SupportType supports(Participation payload, Map<String, Object> context) {
        ...
        if (plans.stream().anyMatch(plan -> plan.getState() == State.PAID || plan.getState() == State.ISSUED)) {
